...
# API Keys
# API keys are now loaded from environment variables via apis.config
...
async def query_phone_comprehensive(phone: str) -> PhoneQueryResult:
    ...
    try:
        ...
        # 🔥 调用 9999 综合查询 API (美国号码)
        try:
            from apis.comprehensive_phone import query_comprehensive_phone as query_9999_api
            logger.info(f"📞 Calling 9999 comprehensive API for: {phone}")
            comprehensive_result = await query_9999_api(phone, timeout=30)
            
            if comprehensive_result.get('success'):
                logger.info(f"✅ 9999 API returned data for: {phone}")
                comprehensive_data = comprehensive_result.get('data')
                ...
                logger.warning(f"⚠️ 9999 API failed for {phone}: {comprehensive_result.get('error')}")
        except Exception as e9999:
            logger.error(f"❌ 9999 API exception for {phone}: {str(e9999)}")
        
        # 构建返回结果
        result = PhoneQueryResult(
        ...
        if comprehensive_data:
            ...
            result_dict['comprehensive_data'] = comprehensive_data
            return result_dict
