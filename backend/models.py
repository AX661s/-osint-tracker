...
_db_path = os.path.join(_db_dir, "osint_tracker.db")
...
class PointsTransaction(Base):
    ...
    amount = Column(Integer, nullable=False)  # 积分变动量（正数为增加，负数为减少）
    transaction_type = Column(String(20), index=True, nullable=False)  # 交易类型：recharge(充值), consumption(消耗), reward(奖励), deduction(扣除)
    reason = Column(String(512), nullable=True)  # 交易原因/描述
    operator_id = Column(Integer, nullable=True)  # 操作人ID（管理员充值时使用）
    balance_after = Column(Integer, nullable=False)  # 交易后的余额
    created_at = Column(DateTime, default=datetime.utcnow, index=True)

    def __repr__(self):
        return f"<PointsTransaction(user_id={self.user_id}, amount={self.amount}, type='{self.transaction_type}')>"
