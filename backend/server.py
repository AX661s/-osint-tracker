...
async def indonesia_phone_lookup(phone: str):
    """
    印尼电话号码查询 - 使用GetContact API + 智能标签解析
    
    Args:
        phone: 印尼电话号码（例如：082271620531）
    
    Returns:
        包含解析后标签的详细信息
    """
    try:
        logger.info(f"🇮🇩 [Indonesia Phone] Looking up phone: {phone}")
        
        # 清理电话号码
        clean_phone = phone.strip().replace('+', '').replace('-', '').replace(' ', '')
        
        # 检查是否是印尼号码
        if not (clean_phone.startswith('62') or clean_phone.startswith('0')):
            return {
                "success": False,
                "error": "Not an Indonesian phone number",
                "message": "请输入有效的印尼电话号码（以62或0开头）"
            }
        
        # 调用GetContact API
        rapidapi_url = f"https://getcontact.p.rapidapi.com/eyex/gc?nomor={clean_phone}"
        headers = {
            'x-rapidapi-host': 'getcontact.p.rapidapi.com',
            'x-rapidapi-key': '3f4bcf0e74msh2f1e9b5504fd778p10f7c6jsn39111f8fffde'
        }
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code != 200:
                logger.error(f"❌ [Indonesia Phone] GetContact API error: {response.status_code}")
                return {
                    "success": False,
                    "error": f"API returned {response.status_code}",
                    "message": "GetContact API查询失败"
                }
            
            data = response.json()
            logger.info(f"✅ [Indonesia Phone] GetContact API success")
            
            # 检查是否有标签数据
            if not data.get('status') or not data.get('data') or not data.get('data', {}).get('tags'):
                return {
                    "success": False,
                    "error": "No tags found",
                    "message": "未找到该号码的标签信息",
            ...
            # 使用智能解析器解析标签
            tags = data['data']['tags']
            parsed_result = parse_indonesia_tags(tags)
            
            logger.info(f"🧠 [Indonesia Phone] Tags parsed: {parsed_result.get('total_unique_tags')} unique tags")
            ...
            final_result = {
                ...
                "gc_pic": data.get('data', {}).get('gc_pic'),
                "whatsapp": data.get('data', {}).get('whatsapp'),
                "whatsapp_pic": data.get('data', {}).get('whatsapp_pic'),
                "whatsapp_business": data.get('data', {}).get('whatsapp_business', False),
                # 智能解析的数据
        ...
        logger.error(f"⏱️ [Indonesia Phone] Timeout")
        return {
            "success": False,
            "error": "Timeout",
            "message": "查询超时，请稍后重试"
        ...
        logger.error(f"❌ [Indonesia Phone] Error: {str(e)}")
        return {
            "success": False,
            "error": str(e),
            "message": "查询失败"
