...
def create_env_file():
    ...
    if confirm("\n是否配置可选的API密钥？"):
        print("\n7. CheckLeaked.cc (Google分析)")
        print("   用途: Google账户深度分析")
        print("   获取: https://checkleaked.cc/")
        checkleaked_key = get_input("   请输入CheckLeaked API密钥（回车跳过）")
        if checkleaked_key:
            env_vars['CHECKLEAKED_API_KEY'] = checkleaked_key
    ...
    with open(env_file, 'w', encoding='utf-8') as f:
        ...
        # 可选的API密钥
        if any(k in env_vars for k in ['CHECKLEAKED_API_KEY', 'WHATSAPP_API_KEY', 'OPENAI_API_KEY', 'SERPAPI_KEY']):
            f.write("# 可选的API密钥\n")
            if 'CHECKLEAKED_API_KEY' in env_vars:
                f.write(f"CHECKLEAKED_API_KEY={env_vars['CHECKLEAKED_API_KEY']}\n")
