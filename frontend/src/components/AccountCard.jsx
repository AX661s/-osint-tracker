...
import { getFieldDisplayName } from "@/utils/fieldMappings";
...
export default function AccountCard(props) {
  ...
  const mainList = normalizeMainFields();
  const bodyFields = (mainList.length > 0)
    ? mainList
    : (Array.isArray(headerFields) ? headerFields.slice(0, 6) : []);
  ...
  return (
    <div className="w-full rounded-2xl border border-border/50 bg-card text-card-foreground shadow overflow-hidden flex flex-col" style={{ minHeight: '380px' }}>
      ...
      <div className="flex items-start gap-3 p-4 sm:p-5 shrink-0">
        <div className="h-10 w-10 shrink-0 rounded-xl overflow-hidden flex items-center justify-center bg-transparent p-0">
          <UniversalLogo
        ...
        <div className="flex-1 min-w-0">
          ...
          {Array.isArray(headerFields) && headerFields.length > 0 && (
            <div className="mt-1.5 flex flex-wrap gap-2">
              {headerFields.slice(0, 6).map((f, idx) => (
                <span key={`${String(f.label)}-${idx}`} className="rounded-full border border-white/10 bg-white/5 px-2.5 py-0.5 text-[11px] text-neutral-200">
                  {String(f.label)}: {String(f.value)}
                </span>
              ))}
            </div>
          )}
    ...
    <div className="ml-auto">
      ...
      {/* Fields section: render only when there are fields or tags */}
      {(bodyFields.length > 0 || tags.length > 0) && (
  ...
  return (
    <div className="space-y-1.5">
      <div className="text-xs uppercase tracking-wider text-neutral-400 flex items-center gap-2">
        <span>{getFieldDisplayName(label)}</span>
        {masked && (
