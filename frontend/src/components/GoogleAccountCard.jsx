    ...
    <GlassCard className="p-0 overflow-hidden" style={{ backgroundColor: 'hsl(var(--background))' }}>
            ...
            <div className="w-16 h-16 rounded-full bg-white/20 backdrop-blur-sm border-2 border-white/40 flex items-center justify-center overflow-hidden">
              {googleData.avatar_url ? (
                <img 
                  ...
                  onError={(e) => {
                    console.log('❌ [Google] Avatar load failed, showing fallback');
                    e.target.parentElement.innerHTML = '<div class="w-full h-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="16" x="2" y="4" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg></div>';
                  }}
                />
              ) : (
      ...
      {showDetails && (
        <div className="p-6 space-y-6">
          {/* 检测是否为公开数据搜索模式 */}
          {googleData.mode === 'public_data_search' && (
            <div className="p-4 rounded-xl bg-amber-500/10 border-2 border-amber-500/30 mb-4">
              <div className="flex items-start gap-3">
                <AlertTriangle className="w-5 h-5 text-amber-500 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="font-semibold text-amber-700 dark:text-amber-400">使用公开数据搜索模式</p>
                  <p className="text-sm text-muted-foreground mt-1">
                    外部 Google API 暂时不可用。以下信息通过公开数据源搜索获得，可能不完整。
                  </p>
                  <p className="text-xs text-muted-foreground mt-2">
                    提示: {googleData.step1_registration?.note || '使用了备用搜索方案'}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-3">
            ...
            <InfoRow 
              ...
              label="账户状态"
              value={
                googleData.step1_registration?.status === 'public_data_only' 
                  ? "公开数据搜索" 
                  : googleData.step1_registration?.email_registered 
                  ? "已注册" 
                  : googleData.step1_registration?.account_exists
                  ? "存在"
                  : "未找到"
              }
            />
