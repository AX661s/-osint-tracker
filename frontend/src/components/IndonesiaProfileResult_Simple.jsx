...
export default function IndonesiaProfileResult({ data, query, onBack }) {
  console.log('🇮🇩 [IndonesiaProfileResult] v2025-11-27-FIXED-v2 loaded');
  console.log('🇮🇩 [IndonesiaProfileResult] 接收到的原始数据:', data);
  console.log('🇮🇩 [IndonesiaProfileResult] 数据类型:', typeof data);
  console.log('🇮🇩 [IndonesiaProfileResult] 数据键:', data ? Object.keys(data) : 'null');
  ...
  const profile = React.useMemo(() => {
    try {
      ...
      if (!data) {
        console.warn('🇮🇩 [IndonesiaProfileResult] 数据为空');
        return {};
      }
      ...
      if (data.data && typeof data.data === 'object') {
        ...
        if (data.data.data && typeof data.data.data === 'object') {
          console.log('🇮🇩 [IndonesiaProfileResult] 检测到双重嵌套结构，使用 data.data.data');
          console.log('🇮🇩 [IndonesiaProfileResult] Profile keys:', Object.keys(data.data.data));
          return data.data.data;
        }

        // 🔥 修复: 如果 data.data 还有 profile 属性 (QueryService 包装的旧后端响应)
        if (data.data.profile && typeof data.data.profile === 'object') {
           console.log('🇮🇩 [IndonesiaProfileResult] 检测到 data.data.profile 结构，使用 data.data.profile');
           console.log('🇮🇩 [IndonesiaProfileResult] Profile keys:', Object.keys(data.data.profile));
        ...
        // 否则使用 data.data
        console.log('🇮🇩 [IndonesiaProfileResult] 使用 data.data 结构');
        console.log('🇮🇩 [IndonesiaProfileResult] Profile keys:', Object.keys(data.data));
        return data.data;
      }
      
      // 🔥 修复: 检查是否有 data.profile (旧版后端结构，被前端 QueryService 包装后)
      if (data.profile && typeof data.profile === 'object') {
        console.log('🇮🇩 [IndonesiaProfileResult] 检测到 data.profile 结构，使用 data.profile');
        console.log('🇮🇩 [IndonesiaProfileResult] Profile keys:', Object.keys(data.profile));
        return data.profile;
      }

      // 如果没有 data.data，但有 basic_info 等字段，说明数据在顶层
      if (data.basic_info || data.contact_info || data.data_breaches) {
        console.log('🇮🇩 [IndonesiaProfileResult] 数据在顶层，直接使用');
        console.log('🇮🇩 [IndonesiaProfileResult] Profile keys:', Object.keys(data));
        return data;
      }
      
      console.warn('🇮🇩 [IndonesiaProfileResult] 无法识别数据结构，返回空对象');
      return {};
    } catch (e) {
      console.error('🇮🇩 [IndonesiaProfileResult] 访问数据时出错:', e);
      return {};
    }
  }, [data]);
  ...
  // 调试输出
  console.log('🇮🇩 [IndonesiaProfileResult] 最终 profile 数据:', {
  ...
  React.useEffect(() => {
    if (!phoneForSocial) {
      console.log('⚠️ [Social APIs] 没有电话号码，跳过社交媒体查询');
      return;
    }
    
    const fetchSocialData = async () => {
      console.log('🔍 [Social APIs] 开始查询社交媒体数据');
      console.log('  - 电话:', phoneForSocial);
      console.log('  - 印尼号码:', isIndonesianPhone);
      ...
      if (isIndonesianPhone) {
        ...
        try {
          ...
          console.log('📘 [Facebook] success:', fbData.success);
          console.log('📘 [Facebook] data:', fbData.data);
          if (fbData.success) {
            setFacebookData(fbData);
            console.log('✅ [Facebook] 数据已设置');
          } else {
            console.log('❌ [Facebook] API 返回失败');
          }
        } catch (e) {
      ...
      try {
        ...
        console.log('📱 [Telegram] success:', tgData.success);
        console.log('📱 [Telegram] data:', tgData.data);
        if (tgData.success) {
          setTelegramData(tgData);
          console.log('✅ [Telegram] 数据已设置');
        } else {
          console.log('❌ [Telegram] API 返回失败');
        }
      } catch (e) {
      ...
      try {
        ...
        console.log('📞 [Truecaller] success:', tcData.success);
        console.log('📞 [Truecaller] data:', tcData.data);
        if (tcData.success) {
          setTruecallerData(tcData);
          console.log('✅ [Truecaller] 数据已设置');
        } else {
          console.log('❌ [Truecaller] API 返回失败');
        }
      } catch (e) {
      ...
      if (isIndonesianPhone) {
        console.log('📞 [Truecaller Acelogic] 开始查询...');
        ...
        try {
          ...
          console.log('📞 [Truecaller Acelogic] success:', tcAceData.success);
          console.log('📞 [Truecaller Acelogic] data:', tcAceData.data);
          if (tcAceData.success) {
            setTruecallerAcelogicData(tcAceData);
            console.log('✅ [Truecaller Acelogic] 数据已设置');
          } else {
            console.log('❌ [Truecaller Acelogic] API 返回失败');
          }
        } catch (e) {
      ...
      try {
        ...
        console.log('👻 [Snapchat] success:', snapData.success);
        console.log('👻 [Snapchat] data:', snapData.data);
        if (snapData.success) {
          setSnapchatData(snapData);
          console.log('✅ [Snapchat] 数据已设置');
        } else {
          console.log('❌ [Snapchat] API 返回失败');
        }
      } catch (e) {
        console.error('👻 [Snapchat] 错误:', e);
      }
      
      console.log('🏁 [Social APIs] 所有查询完成');
    };
                          ...
                          >
                            ...
                            <span className="flex-1">
                              <div className="font-semibold flex items-center gap-2">
                                Telegram
                                <span className="text-xs bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded">已验证</span>
                              </div>
                        ...
                        {socialLoading.truecaller ? (
                          ...
                          >
                            ...
                            <span className="flex-1">
                              <div className="font-semibold flex items-center gap-2">
                                WhatsApp
                                <span className="text-xs bg-green-500/20 text-green-400 px-2 py-0.5 rounded">已验证</span>
                              </div>
                              <div className="text-sm opacity-75 text-foreground">
                                该号码已关联 WhatsApp 账户
                              </div>
