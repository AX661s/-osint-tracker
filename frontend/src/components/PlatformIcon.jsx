...
export const PlatformIcon = ({ platform, url, className = "w-5 h-5" }) => {
  ...
  // IPQualityScore - 使用真实品牌图标（通过代理），放大1.2倍
  if (!colorIconMap['ipqualityscore']) {
    colorIconMap['ipqualityscore'] = (
      <img
        ...
        referrerPolicy="no-referrer"
        crossOrigin="anonymous"
        style={{ transform: 'scale(1.2)', objectFit: 'contain' }}
        onError={(e) => {
          // 如果加载失败，回退到自定义图标
  ...
  // Melissa - 使用真实品牌图标（通过代理）
  if (!colorIconMap['melissa']) {
    colorIconMap['melissa'] = (
      <img
        src="/api/logo/melissa.com"
        alt="Melissa"
        className={className}
        loading="lazy"
        referrerPolicy="no-referrer"
        ...
        onError={(e) => {
          ...
          e.target.parentElement.innerHTML = `
            <svg class="${className}" viewBox="0 0 24 24" fill="none">
              ...
              <text x="12" y="16" font-family="Arial" font-size="14" font-weight="bold" text-anchor="middle" fill="#fff">M</text>
            </svg>
          `;
        }}
      />
