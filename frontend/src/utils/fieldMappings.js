...
export const getFieldDisplayName = (fieldName) => {
  ...
  const camelToSnake = raw.replace(/([a-z])([A-Z])/g, '$1_$2');
  const normalized = camelToSnake.replace(/\s+/g, '_').toLowerCase();
  return FIELD_DISPLAY_MAP[normalized] || FIELD_DISPLAY_MAP[raw] || raw;
};
...
export const extractWebsiteUrl = (data) => {
  ...
  const platformDomainMap = {
    ...
    'phone_lookup': 'https://www.melissa.com',
    'phone_lookup_3008': 'https://www.melissa.com',
...
export const extractAvatarUrl = (data) => {
  ...
  const readVal = (val) => {
    ...
    if (typeof val === 'object') {
      ...
      if (Object.prototype.hasOwnProperty.call(val, 'value') && typeof val.value === 'string') return normalize(val.value);
      // 可能是 { url: ... }
