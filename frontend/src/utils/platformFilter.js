...
import { hasNonMetadataData, METADATA_FIELDS, isNonEmptyArray, isValidObject } from './common';
...
export class PlatformFilter {
  ...
  static hasValidData(platform) {
    ...
    if (isNonEmptyArray(platform.spec_format)) {
      const specItem = platform.spec_format[0];
      if (isValidObject(specItem) && hasNonMetadataData(specItem)) {
        return true;
      }
    }

    // 检查 data 字段是否有数据
    if (isValidObject(platform.data) && hasNonMetadataData(platform.data)) {
      return true;
    }

    // 检查平台本身是否有除了元数据外的其他字段
    return hasNonMetadataData(platform, METADATA_FIELDS);
  }
