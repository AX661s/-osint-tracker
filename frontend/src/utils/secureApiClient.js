...
class SecureApiClient {
  constructor(config = {}) {
    this.baseURL = config.baseURL || process.env.REACT_APP_API_URL || (process.env.NODE_ENV === 'production' ? '/api' : 'http://localhost:8000/api');
    this.timeout = config.timeout || 180000; // 180秒超时（印尼API需要较长时间）
    this.maxRetries = config.maxRetries || 0; // 不重试，只调用一次
